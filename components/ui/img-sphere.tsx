...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
    ...
    const generateSpherePositions = useCallback((): SphericalPosition[] => {
        ...
        for (let i = 0; i < imageCount; i++) {
            ...
            // Randomization removed to prevent overlapping clusters
    ...
    const calculateWorldPositions = useCallback((): WorldPosition[] => {
        ...
        // Collision detection removed for smoother performance and to prevent jitter
        return positions;
    ...
    const renderImageNode = useCallback((image: ImageData, index: number) => {
        ...
        // Total scale = depth/position scale * hover scale
        const totalScale = position.scale * (isHovered ? 1.2 / position.scale : 1); // 1.2 absolute scale on hover, or relative?
        // Let's stick to the previous hover logic:
        // old: finalScale = isHovered ? Math.min(1.2, 1.2 / position.scale) : 1;
        // old usage: width = baseImageSize * position.scale, transform scale(finalScale)
        // new usage: width = baseImageSize, transform scale(position.scale * finalScale)

        const hoverMultiplier = isHovered ? Math.min(1.5, 1.2 / position.scale) : 1;
        const finalRenderScale = position.scale * hoverMultiplier;
        ...
        return (
            <div
                ...
                style={{
                    ...
                    // Use translate3d for GPU acceleration - relative to center
                    transform: `translate3d(${position.x}px, ${position.y}px, 0) translate(-50%, -50%) scale(${finalRenderScale})`,
